...
# note: unused - thought lambda's might be faster --rlmitchell
'''
...
def create_filters(date=None, start_date=None, end_date=None,
    ...
    if date:
        filters.append(lambda ca: ca.time and
                       str(ca.time).split()[0] == str(date))

    if start_date and end_date:
        filters.append(lambda ca: ca.time and
                       (ca.time.date() >= start_date) and
                       (ca.time.date() <= end_date))
    else:
        if start_date:
            filters.append(lambda ca: ca.time and
                           (ca.time.date() >= start_date))
        if end_date:
            filters.append(lambda ca: ca.time and
                           (ca.time.date() <= end_date))

    if distance_min and distance_max:
        filters.append(lambda ca: float(ca.distance) >= float(distance_min) and
                       float(ca.distance) <= float(distance_max))
    else:
        if distance_min:
            filters.append(lambda ca: float(ca.distance)
                           >= float(distance_min))
        if distance_max:
            filters.append(lambda ca: float(ca.distance)
                           <= float(distance_max))
    ...
    if velocity_min and velocity_max:
        filters.append(lambda ca: float(ca.velocity) >= float(velocity_min) and
                       float(ca.velocity) <= float(velocity_max))
    else:
        if velocity_min:
            filters.append(lambda ca: float(ca.velocity)
                           >= float(velocity_min))
        if velocity_max:
            filters.append(lambda ca: float(ca.velocity)
                           <= float(velocity_max))
    ...
    if diameter_min and diameter_max:
        filters.append(lambda ca: float(ca.neo.diameter)
                       >= float(diameter_min) and float(ca.neo.diameter)
                       <= float(diameter_max))
    else:
        if diameter_min:
            filters.append(lambda ca: float(ca.neo.diameter)
    ...
    if hazardous is not None:
        if hazardous is True:
            filters.append(lambda ca: ca.neo.hazardous is True)
        if hazardous is False:
            filters.append(lambda ca: ca.neo.hazardous is False)
...
def limit(iterator, n=None):
    ...
    # note - tried returning tuple resulted in exception  --rlmitchell
